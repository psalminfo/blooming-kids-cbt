...
async function fetchRecallRequests(forceRefresh = false) {
    try {
        ...
        const requests = snapshot.docs.map(doc => ({ 
            id: doc.id, 
            ...doc.data(),
            requestDate: doc.data().requestDate?.toDate ? doc.data().requestDate.toDate() : new Date(doc.data().requestDate || new Date()),
        ...
        const recallView = document.getElementById('recall-requests-view');
        if (recallView && !recallView.classList.contains('hidden')) {
        ...
        if (container) {
            container.innerHTML = `
                <div class="text-center py-10 text-red-600">
                    ...
                    <p class="text-sm mt-2">${error.message}</p>
                    <button onclick="fetchRecallRequests(true)" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                        Try Again
                    </button>
                </div>
            `;
