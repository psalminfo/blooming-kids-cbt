...
// NEW: This function renders the combined student directory
async function renderStudentDirectoryView(mainContent) {
    if (!mainContent) return;
    mainContent.innerHTML = `
        <div class="p-6 bg-gray-100 min-h-screen">
            <h2 class="text-3xl font-bold text-gray-800 mb-6">Student Directory</h2>
            <div id="student-directory-list" class="space-y-4">
                <p class="text-center text-gray-500 mt-4">Loading students...</p>
            </div>
        </div>
    `;

    const studentDirectoryList = mainContent.querySelector('#student-directory-list');
    const studentsCollection = collection(db, "students");
    const pendingStudentsCollection = collection(db, "pending_students");

    const [studentsSnapshot, pendingSnapshot] = await Promise.all([
        getDocs(studentsCollection),
        getDocs(pendingStudentsCollection)
    ]);

    let students = studentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), status: 'approved' }));
    let pendingStudents = pendingSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), status: 'pending' }));
    
    let allStudents = [...students, ...pendingStudents];
    
    studentDirectoryList.innerHTML = '';
    if (allStudents.length === 0) {
        studentDirectoryList.innerHTML = `<p class="text-center text-gray-500 mt-4">No students found in the directory.</p>`;
        return;
    }
    ...
    allStudents.forEach(student => {
        ...
        if (student.status === 'pending') {
            statusTag = `<span class="bg-yellow-200 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded-full ml-2">Awaiting Approval</span>`;
        }

        studentItem.innerHTML = `
            <div class="flex-1">
                <p class="text-lg font-semibold text-gray-800">${capitalize(student.studentName)} ${statusTag}</p>
                ...
                <p class="text-sm text-gray-600"><strong>Email:</strong> ${student.tutorId || 'N/A'}</p>
            </div>
        `;
        studentDirectoryList.appendChild(studentItem);
    });
}
...
async function renderPendingApprovalView(mainContent) {
    ...
    mainContent.innerHTML = `
        <div class="p-6 bg-gray-100 min-h-screen">
            ...
            <button id="addStudentBtn" class="bg-blue-500 text-white px-4 py-2 rounded-md shadow hover:bg-blue-600 transition-colors mb-4">Add New Student</button>
        ...
        snapshot.forEach((doc) => {
            ...
            studentItem.innerHTML = `
                <div class="flex-1 space-y-1">
                    ...
                    <p class="text-sm text-gray-600"><strong>Tutor:</strong> ${student.tutorName || 'N/A'}</p>
                    <p class="text-sm text-gray-600"><strong>Class:</strong> ${student.class || 'N/A'}</p>
                    <p class="text-sm text-gray-600"><strong>Email:</strong> ${student.tutorEmail || 'N/A'}</p>
                </div>
                <div class="flex space-x-2">
                    <button class="bg-green-500 text-white px-4 py-2 rounded-md shadow hover:bg-green-600 transition-colors approve-btn" data-id="${studentId}" data-tutor-id="${student.tutorEmail}">Approve</button>
                    <button class="bg-blue-500 text-white px-4 py-2 rounded-md shadow hover:bg-blue-600 transition-colors edit-btn" data-id="${studentId}" data-tutor-id="${student.tutorEmail}">Edit</button>
                    <button class="bg-red-500 text-white px-4 py-2 rounded-md shadow hover:bg-red-600 transition-colors reject-btn" data-id="${studentId}">Delete</button>
                </div>
            `;
    ...
    const addStudentBtn = mainContent.querySelector('#addStudentBtn');
    if (addStudentBtn) {
...
function initializeManagementPanel() {
    ...
    const navStudentDirectory = document.getElementById('navStudentDirectory');
    ...
    navStudentDirectory.addEventListener('click', () => {
        document.querySelectorAll('nav button').forEach(btn => btn.classList.remove('active'));
        navStudentDirectory.classList.add('active');
        renderStudentDirectoryView(mainContent);
    });
}
...
async function handleRejection(studentId) {
    try {
        ...
        showModal('Error deleting student. Please try again.');
    }
}
...
async function handleAddStudent(newStudentData) {
    try {
        ...
        showModal('New student added successfully! They are now awaiting approval.');
    } catch (error) {
        console.error("Error adding new student:", error);
        showModal('Error adding new student. Please try again.');
    }
}
...
function showAddStudentModal() {
    ...
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            ...
            <form id="add-student-form">
                <div class="mb-4">
                    <label class="block text-gray-700">Student Name</label>
                    <input type="text" id="add-studentName" class="mt-1 p-2 w-full border rounded-md" required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700">Class</label>
                    <input type="text" id="add-class" class="mt-1 p-2 w-full border rounded-md" required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700">Tutor Name</label>
                    <input type="text" id="add-tutorName" class="mt-1 p-2 w-full border rounded-md">
                </div>
                 <div class="mb-4">
                    <label class="block text-gray-700">Tutor Email</label>
                    <input type="email" id="add-tutorEmail" class="mt-1 p-2 w-full border rounded-md" required>
                </div>
                <div class="flex justify-end space-x-2">
                    <button type="button" id="close-add-modal-btn" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400">Cancel</button>
    ...
    document.getElementById('add-student-form').onsubmit = async (e) => {
        ...
        const newStudentData = {
            studentName: document.getElementById('add-studentName').value,
            class: document.getElementById('add-class').value,
            tutorName: document.getElementById('add-tutorName').value,
            tutorEmail: document.getElementById('add-tutorEmail').value,
        };
        await handleAddStudent(newStudentData);
        modal.style.display = 'none';
    };
        ...
        onSnapshot(staffDocRef, async (docSnap) => {
            if (docSnap.exists() && docSnap.data()?.active) {
                ...
                const allNavItems = {
                    ...
                    directory: { label: 'Student Directory', fn: renderStudentDirectoryView, icon: 'fa-address-book' }
                };
