...
function showAssignStudentModal() {
    ...
    document.getElementById('assign-student-form').addEventListener('submit', async (e) => {
        ...
        const newStudentData = {
            ...
            parentEmail: form.elements['assign-parentEmail'].value || '',
...
const navigationGroups = {
    ...
    "tutorManagement": {
        ...
        items: [
             { id: "navMasterPortal", label: "Management Portal", icon: "fas fa-chalkboard-teacher", fn: renderMasterPortal },
...
// ======================================================
// SECTION: MANAGEMENT PORTAL (MASTER VIEW)
// ======================================================

/**
 * MANAGEMENT PORTAL - Master View
 * 
 * This section provides a unified view of all tutors and their students
 * with performance scoring capabilities.
 */

// -------------------- Data Fetching --------------------

async function fetchMasterData(forceRefresh = false) {
    console.log("📊 Fetching master portal data...");
    
    try {
        // Check cache first unless forced refresh
        if (!forceRefresh && sessionCache.masterData && sessionCache.masterData.length > 0) {
            console.log("✅ Using cached master data:", sessionCache.masterData.length, "records");
            return sessionCache.masterData;
        }

        // Show loading state if container exists
        const container = document.getElementById('master-portal-table-body');
        if (container) {
            container.innerHTML = `
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-600">Loading master data...</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        const studentsQuery = query(
            collection(db, "students"),
            where("status", "in", ["active", "approved", null])
        );
        const studentsSnapshot = await getDocs(studentsQuery);
        ...
        // Create student lookup by tutor email
        const studentsByTutor = {};
        ...
        tutors.forEach(tutor => {
            ...
            // If tutor has no students, still include them with empty student data
            if (tutorStudents.length === 0) {
                masterData.push({
                    ...
                    performanceHistory: tutor.performanceHistory || [],
                    studentId: null,
                    studentName: null,
                    parentName: null,
                    parentPhone: null,
                    parentEmail: null,
                    scheduleSummary: 'No students assigned',
                    grade: null,
                    subjects: [],
                    hasStudents: false
                });
        ...
        // Sort by tutor name then student name
        masterData.sort((a, b) => {
            if (a.tutorName !== b.tutorName) {
                return a.tutorName.localeCompare(b.tutorName);
            }
            // Tutors with no students come after tutors with students
            if (!a.hasStudents && b.hasStudents) return 1;
            if (a.hasStudents && !b.hasStudents) return -1;
            // Both have students or both don't have students, sort by student name
            return (a.studentName || '').localeCompare(b.studentName || '');
        });

        // Cache the result
        sessionCache.masterData = masterData;
        
        console.log(`✅ Master data prepared: ${masterData.length} rows`);
        return masterData;
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="text-red-600">
                            ...
                            <p class="font-semibold">Failed to load master data</p>
                            <p class="text-sm mt-2">${error.message}</p>
                            <button onclick="refreshMasterData()" 
...
function formatScheduleSummary(days, time) {
    if (!days || days === 'TBD') return 'Schedule TBD';
    
    // Clean up days format
    let cleanDays = days;
    ...
    if (!time || time === 'TBD') return cleanDays;
    return `${cleanDays} • ${time}`;
}

// -------------------- Render Master Portal --------------------

async function renderMasterPortal(container) {
    console.log("🎨 Rendering Master Portal...");
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="p-6 border-b bg-gradient-to-r from-green-50 to-blue-50">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Unified view of all tutors and their students</p>
                    </div>
                    ...
                    <div class="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <div class="relative flex-1 md:flex-none md:w-64">
                            ...
                            <input type="text" 
                                   id="master-portal-search" 
                                   placeholder="Search tutors, students, parents..." 
                                   class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        </div>
                        
                        <!-- Filter Dropdown -->
                        <select id="master-portal-filter" 
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-gray-50 border-b">
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <p class="text-sm text-gray-600">Total Tutors</p>
                    <p id="stat-total-tutors" class="text-2xl font-bold text-blue-600">0</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <p class="text-sm text-gray-600">Total Students</p>
                    <p id="stat-total-students" class="text-2xl font-bold text-green-600">0</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <p class="text-sm text-gray-600">Avg Performance</p>
                    <p id="stat-avg-performance" class="text-2xl font-bold text-purple-600">0</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <p class="text-sm text-gray-600">Tutors w/ Students</p>
                    <p id="stat-active-tutors" class="text-2xl font-bold text-orange-600">0</p>
                </div>
            </div>
            ...
            <div class="overflow-x-auto" style="max-height: 600px;">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody id="master-portal-table-body" class="bg-white divide-y divide-gray-200">
                        <tr>
            ...
            <!-- Footer -->
            <div class="px-6 py-4 bg-gray-50 border-t text-sm text-gray-600 flex justify-between items-center">
                <span id="record-count">Loading records...</span>
                <button onclick="exportMasterDataToCSV()" class="text-green-600 hover:text-green-800">
    ...
    // Add event listeners
    document.getElementById('refresh-master-btn').addEventListener('click', () => {
        refreshMasterData();
    });

    document.getElementById('master-portal-search').addEventListener('input', (e) => {
        filterMasterData(e.target.value);
    });

    document.getElementById('master-portal-filter').addEventListener('change', (e) => {
        filterMasterData(document.getElementById('master-portal-search').value, e.target.value);
    });
...
async function refreshMasterData() {
    try {
        currentMasterData = await fetchMasterData(true);
        renderMasterTable(currentMasterData);
        updateMasterStats(currentMasterData);
    } catch (error) {
        console.error("Error refreshing master data:", error);
    }
}

function filterMasterData(searchTerm = '', filterType = 'all') {
    ...
    let filtered = currentMasterData.filter(row => {
        // Apply search filter
        if (searchLower) {
        ...
        switch (filterType) {
            case 'has-students':
                return row.hasStudents;
            case 'no-students':
                return !row.hasStudents;
            case 'high-performers':
                return row.performanceScore >= 90;
            default:
                return true;
        }
    });
...
function renderMasterTable(data) {
    const tbody = document.getElementById('master-portal-table-body');
    if (!tbody) return;
    ...
    tbody.innerHTML = data.map(row => {
        ...
        let performanceBadgeColor = 'bg-gray-100 text-gray-800';
        if (row.performanceScore >= 90) performanceBadgeColor = 'bg-green-100 text-green-800';
        else if (row.performanceScore >= 75) performanceBadgeColor = 'bg-blue-100 text-blue-800';
        else if (row.performanceScore >= 60) performanceBadgeColor = 'bg-yellow-100 text-yellow-800';
        else if (row.performanceScore > 0) performanceBadgeColor = 'bg-orange-100 text-orange-800';

        // Format parent contact
        const parentContact = row.hasStudents ? `
            <div class="flex flex-col">
                <span class="font-medium">${row.parentName}</span>
                ${row.parentPhone ? `
                    <a href="tel:${row.parentPhone}" 
                       class="text-sm text-blue-600 hover:text-blue-800 flex items-center mt-1">
        ...
        ` : '<span class="text-gray-400 italic">No student assigned</span>';

        // Format schedule
        const schedule = row.hasStudents ? `
            <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-sm">
                ${row.scheduleSummary}
            </span>
        ` : '<span class="text-gray-400 italic">N/A</span>';

        // Format subjects as badges
        const subjects = row.hasSubjects && row.subjects.length > 0 ? 
            `<div class="flex flex-wrap gap-1 mt-2">
                ${row.subjects.slice(0, 3).map(subject => 
                    `<span class="px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full text-xs">${subject}</span>`
                ).join('')}
                ${row.subjects.length > 3 ? `<span class="text-xs text-gray-500">+${row.subjects.length - 3}</span>` : ''}
            </div>` : '';
        ...
        return `
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-4">
                    <div class="flex flex-col">
                        <div class="flex items-center">
                            ...
                            <span class="font-medium text-gray-900">${row.tutorName}</span>
                        </div>
                        <span class="text-xs text-gray-500 mt-1">${row.tutorEmail}</span>
                        ${subjects}
                    </div>
                </td>
                ...
                <td class="px-6 py-4">
                    ${row.hasStudents ? `
                        <div class="flex flex-col">
                            ...
                            <span class="text-xs text-gray-500 mt-1">Grade: ${row.grade}</span>
                        </div>
                    ` : `
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center">
                        <span class="px-3 py-1 rounded-full text-sm font-medium ${performanceBadgeColor}">
                ...
                <!-- Actions Column -->
                <td class="px-6 py-4">
                    <button onclick="showRateTutorModal('${row.tutorId}', '${row.tutorName}', ${row.performanceScore || 0})" 
                            class="bg-purple-600 text-white px-3 py-2 rounded-lg hover:bg-purple-700 transition-colors text-sm flex items-center">
...
function updateMasterStats(data) {
    // Calculate unique tutors and students
    const uniqueTutors = new Set(data.map(row => row.tutorId)).size;
    const uniqueStudents = new Set(data.filter(row => row.hasStudents).map(row => row.studentId)).size;
    
    // Calculate average performance (only for tutors with scores)
    const tutorsWithScores = new Map();
    data.forEach(row => {
        if (!tutorsWithScores.has(row.tutorId)) {
            tutorsWithScores.set(row.tutorId, {
                score: row.performanceScore || 0,
                count: 1
            });
        }
    });
    ...
    let totalScore = 0;
    tutorsWithScores.forEach(tutor => {
        totalScore += tutor.score;
    });
    
    const avgPerformance = tutorsWithScores.size > 0 
        ? Math.round(totalScore / tutorsWithScores.size) 
        : 0;

    // Count tutors with students
    const tutorsWithStudents = new Set(
        data.filter(row => row.hasStudents).map(row => row.tutorId)
    ).size;

    document.getElementById('stat-total-tutors').textContent = uniqueTutors;
    ...
    document.getElementById('stat-active-tutors').textContent = tutorsWithStudents;
...
function updateRecordCount(count) {
    ...
    if (recordSpan) {
        recordSpan.textContent = `Showing ${count} record${count !== 1 ? 's' : ''}`;
    }
}

// -------------------- Rate Tutor Modal --------------------

function showRateTutorModal(tutorId, tutorName, currentScore) {
    console.log("Opening rate modal for:", tutorId, tutorName);
    
    // Remove existing modal if any
    const existingModal = document.getElementById('rate-tutor-modal');
    if (existingModal) existingModal.remove();

    const modalHtml = `
        <div id="rate-tutor-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
            <div class="relative bg-white w-full max-w-md rounded-lg shadow-2xl">
                <!-- Modal Header -->
                <div class="flex items-center justify-between p-6 border-b">
                    ...
                    <button onclick="closeManagementModal('rate-tutor-modal')" 
                ...
                <div class="p-6">
                    <div class="mb-6">
                        <p class="text-gray-700 mb-2">Tutor: <span class="font-bold">${tutorName}</span></p>
                        <p class="text-sm text-gray-500 mb-4">Current Score: ${currentScore || 'Not rated'}</p>
                        
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Performance Score (0-100) <span class="text-red-500">*</span>
                        </label>
                        
                        <div class="mb-4">
                            <input type="range" 
                                   id="performance-score-slider" 
                                   min="0" 
                                   max="100" 
                                   value="${currentScore || 75}" 
                                   step="1"
                        ...
                        <div class="mb-6 text-center">
                            <span id="selected-score" class="text-4xl font-bold text-purple-600">
                                ${currentScore || 75}
                            </span>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="performance-notes" 
                                      rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                      placeholder="Add notes about this performance rating..."></textarea>
                        </div>

                        <div class="mb-4 p-4 bg-purple-50 rounded-lg">
                            <p class="text-sm text-purple-700 flex items-start">
                                <i class="fas fa-info-circle mr-2 mt-0.5"></i>
                                <span>Scores will be saved to the tutor's record and visible in the performance history.</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 p-6 border-t bg-gray-50 rounded-b-lg">
                    ...
                    <button onclick="submitTutorRating('${tutorId}', '${tutorName}')" 
                            id="submit-rating-btn"
                            class="px-5 py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center">
    ...
    const slider = document.getElementById('performance-score-slider');
    ...
    slider.addEventListener('input', function() {
        ...
        if (score >= 90) {
            scoreDisplay.className = 'text-4xl font-bold text-green-600';
        } else if (score >= 75) {
            scoreDisplay.className = 'text-4xl font-bold text-blue-600';
        } else if (score >= 60) {
            scoreDisplay.className = 'text-4xl font-bold text-yellow-600';
        } else {
            scoreDisplay.className = 'text-4xl font-bold text-orange-600';
        }
    });
}
...
async function submitTutorRating(tutorId, tutorName) {
    const score = parseInt(document.getElementById('performance-score-slider').value);
    ...
    try {
        ...
        const tutorData = tutorDoc.data();
        const performanceHistory = tutorData.performanceHistory || [];
        
        // Add new performance record
        const newRecord = {
            score: score,
            notes: notes,
            ratedBy: window.userData?.name || window.userData?.email || 'Management',
            ratedByEmail: window.userData?.email || 'management',
            ratedAt: Timestamp.now(),
            previousScore: tutorData.performanceScore || 0
        };

        // Update tutor document
        await updateDoc(tutorRef, {
            performanceScore: score,
            performanceHistory: [...performanceHistory, newRecord],
            lastPerformanceRating: Timestamp.now(),
            lastUpdated: Timestamp.now(),
            updatedBy: window.userData?.email || 'management'
        });

        // Log the event
        console.log(`✅ Performance rating saved for ${tutorName}: ${score}`);

        // Show success message
        alert(`✅ Performance rating saved for ${tutorName}: ${score}/100`);

        // Close modal
        closeManagementModal('rate-tutor-modal');

        // Invalidate cache and refresh
        sessionCache.masterData = null;
        await refreshMasterData();
...
function exportMasterDataToCSV() {
    ...
    // Prepare CSV rows
    const rows = currentMasterData.map(row => [
        `"${row.tutorName}"`,
        `"${row.tutorEmail}"`,
        row.performanceScore || 0,
        `"${row.studentName || ''}"`,
        `"${row.grade || ''}"`,
        `"${row.subjects ? row.subjects.join(', ') : ''}"`,
        `"${row.parentName || ''}"`,
        `"${row.parentPhone || ''}"`,
        `"${row.parentEmail || ''}"`,
        `"${row.scheduleSummary || ''}"`,
        row.hasStudents ? 'Yes' : 'No'
    ]);
    ...
    link.href = url;
    link.download = `Management_Portal_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
    URL.revokeObjectURL(url);

    console.log(`📥 Exported ${currentMasterData.length} records to CSV`);
}

// -------------------- Add to Navigation --------------------

// Add this to your navigation configuration
const masterPortalNavItem = {
    id: "navMasterPortal",
    label: "Management Portal",
    icon: "fas fa-chalkboard-teacher",
    fn: renderMasterPortal
};

// To integrate with your existing navigation, add this to the appropriate group
// For example, add to "tutorManagement" group items:
/*
items: [
    { id: "navMasterPortal", label: "Management Portal", icon: "fas fa-chalkboard-teacher", fn: renderMasterPortal },
    { id: "navTutorManagement", label: "Tutor Directory", icon: "fas fa-users", fn: renderManagementTutorView },
...
window.renderMasterPortal = renderMasterPortal;
window.refreshMasterData = refreshMasterData;
window.showRateTutorModal = showRateTutorModal;
window.submitTutorRating = submitTutorRating;
