...
async function renderTutorManagementPanel(container) {
    container.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Report Submission:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span id="report-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Tutors Can Add Students:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="tutor-add-toggle" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after-w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span id="tutor-add-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Enable Summer Break:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="summer-break-toggle" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span id="summer-break-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
                
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Student Edit/Delete:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="edit-delete-toggle" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span id="edit-delete-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Show Student Fees:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="show-fees-toggle" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span id="show-fees-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
                <label class="flex items-center">
                    <span class="text-gray-700 font-semibold mr-4">Direct Student Add:</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="bypass-approval-toggle" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span id="bypass-approval-status-label" class="ml-3 text-sm font-medium"></span>
                    </label>
                </label>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <h2 class="text-2xl font-bold text-green-700 mb-4">Search & Manage Tutors</h2>
            <input type="text" id="tutorSearchInput" placeholder="Search tutors by name or email..." class="w-full p-2 mb-4 border rounded">
            <div id="tutorListContainer" class="space-y-4"></div>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-bold text-green-700 mb-4">Individual Tutor Details</h2>
            <div id="tutorDetailsContainer" class="p-4 border rounded-md min-h-[200px] text-gray-500">Select a tutor to view details.</div>
        </div>
    `;

    // Fetch initial settings and set up listeners
    onSnapshot(doc(db, "settings", "global_settings"), (docSnap) => {
        if (docSnap.exists()) {
            ...
            if (reportToggle) {
                ...
                document.getElementById('report-status-label').textContent = data.isReportEnabled ? 'Enabled' : 'Disabled';
            }
            const tutorAddToggle = document.getElementById('tutor-add-toggle');
            if (tutorAddToggle) {
                tutorAddToggle.checked = !!data.isTutorAddEnabled;
                document.getElementById('tutor-add-status-label').textContent = data.isTutorAddEnabled ? 'Enabled' : 'Disabled';
            }
            const summerBreakToggle = document.getElementById('summer-break-toggle');
            if (summerBreakToggle) {
                summerBreakToggle.checked = !!data.isSummerBreakEnabled;
                document.getElementById('summer-break-status-label').textContent = data.isSummerBreakEnabled ? 'Enabled' : 'Disabled';
            }
            ...
            if (editDeleteToggle) {
                ...
                document.getElementById('edit-delete-status-label').textContent = data.showEditDeleteButtons ? 'Enabled' : 'Disabled';
            }
            const showFeesToggle = document.getElementById('show-fees-toggle');
            if (showFeesToggle) {
                showFeesToggle.checked = !!data.showTotalStudentFee;
                document.getElementById('show-fees-status-label').textContent = data.showTotalStudentFee ? 'Enabled' : 'Disabled';
            }
            const bypassApprovalToggle = document.getElementById('bypass-approval-toggle');
            if (bypassApprovalToggle) {
                bypassApprovalToggle.checked = !!data.bypassPendingApproval;
                document.getElementById('bypass-approval-status-label').textContent = data.bypassPendingApproval ? 'Enabled' : 'Disabled';
            }
        }
    });

    // Setup event listeners for the new toggles
    document.getElementById('edit-delete-toggle')?.addEventListener('change', async (e) => {
        await updateDoc(doc(db, "settings", "global_settings"), {
            showEditDeleteButtons: e.target.checked
        });
    });
    document.getElementById('show-fees-toggle')?.addEventListener('change', async (e) => {
        await updateDoc(doc(db, "settings", "global_settings"), {
            showTotalStudentFee: e.target.checked
        });
    });
    ...
    // Setup listeners for the existing toggles (re-added for a full-file example)
    document.getElementById('report-toggle')?.addEventListener('change', async (e) => {
        await updateDoc(doc(db, "settings", "global_settings"), {
            isReportEnabled: e.target.checked
        });
    });
    document.getElementById('tutor-add-toggle')?.addEventListener('change', async (e) => {
        await updateDoc(doc(db, "settings", "global_settings"), {
            isTutorAddEnabled: e.target.checked
        });
    });
    document.getElementById('summer-break-toggle')?.addEventListener('change', async (e) => {
        await updateDoc(doc(db, "settings", "global_settings"), {
...
// Existing functions for this section (for context)
async function loadTutors() {
    const tutorListContainer = document.getElementById('tutorListContainer');
    tutorListContainer.innerHTML = '<p class="text-center text-gray-500">Loading tutors...</p>';
    try {
        ...
        snapshot.forEach(doc => {
            ...
            tutorCard.className = 'bg-gray-100 p-4 rounded-lg shadow-sm cursor-pointer hover:bg-gray-200 transition-colors duration-200';
            tutorCard.innerHTML = `
                <div class="flex justify-between items-center">
                    <h3 class="font-bold text-gray-800">${tutor.name}</h3>
        ...
        tutorListContainer.innerHTML = '<p class="text-center text-red-500">Failed to load tutors.</p>';
    }
}

async function displayTutorDetails(tutor) {
    const container = document.getElementById('tutorDetailsContainer');
    container.innerHTML = `<p class="text-center text-gray-500">Loading details...</p>`;
    activeTutorId = tutor.email;

    try {
        // Fetch students under this tutor
        const studentsQuery = query(collection(db, "student_results"), where("tutorEmail", "==", activeTutorId));
        const studentsSnapshot = await getDocs(studentsQuery);

        const studentsList = studentsSnapshot.docs.map(studentDoc => {
            const student = studentDoc.data();
            return `<li>${student.studentName} (${student.grade}) - Fee: ₦${student.fee}</li>`;
        }).join('');

        const tutorRoleRef = doc(db, "staff", activeTutorId);
        const tutorRoleSnap = await getDoc(tutorRoleRef);
        const tutorRole = tutorRoleSnap.exists() ? tutorRoleSnap.data().role : 'tutor';

        container.innerHTML = `
            <div class="p-4 border rounded-lg shadow-md bg-white">
                <h3 class="text-xl font-bold mb-2 text-blue-700">${tutor.name}</h3>
                <p><strong>Email:</strong> ${tutor.email}</p>
                <p><strong>Phone:</strong> ${tutor.phone || 'N/A'}</p>
                <p><strong>Registered:</strong> ${new Date(tutor.createdAt.seconds * 1000).toLocaleDateString()}</p>
                <p class="mt-2"><strong>Current Role:</strong>
                <select id="tutor-role-select" class="ml-2 p-1 border rounded">
                    <option value="pending" ${tutorRole === 'pending' ? 'selected' : ''}>Pending</option>
                    <option value="tutor" ${tutorRole === 'tutor' ? 'selected' : ''}>Tutor</option>
                    <option value="manager" ${tutorRole === 'manager' ? 'selected' : ''}>Manager</option>
                    <option value="director" ${tutorRole === 'director' ? 'selected' : ''}>Director</option>
                    <option value="admin" ${tutorRole === 'admin' ? 'selected' : ''}>Admin</option>
                </select>
                <button id="update-role-btn" class="bg-blue-500 text-white px-3 py-1 rounded ml-2 text-sm hover:bg-blue-600">Update Role</button>
                </p>
                <h4 class="font-semibold text-lg mt-4">Students Under Management (${studentsSnapshot.size})</h4>
                <ul class="list-disc list-inside mt-2 text-gray-700">
                    ${studentsList || '<li>No students found.</li>'}
                </ul>
                <div class="mt-6 flex gap-4">
                    <button id="delete-tutor-btn" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition-colors">Delete Tutor</button>
                    <button id="reset-password-btn" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 transition-colors">Reset Password</button>
                </div>
            </div>
        `;
        // Setup listeners for the detail panel buttons
        document.getElementById('update-role-btn').addEventListener('click', async () => {
            const newRole = document.getElementById('tutor-role-select').value;
            await updateStaffPermissions(activeTutorId, newRole);
        ...
        document.getElementById('delete-tutor-btn').addEventListener('click', async () => {
            if (confirm("Are you sure you want to delete this tutor? This cannot be undone.")) {
                try {
                    await deleteDoc(doc(db, "tutors", activeTutorId));
                    await deleteDoc(doc(db, "staff", activeTutorId));
                    alert("Tutor deleted successfully.");
                    loadTutors(); // Reload the list
                    document.getElementById('tutorDetailsContainer').innerHTML = `<p class="text-center text-gray-500">Tutor deleted.</p>`;
                } catch (error) {
        ...
        document.getElementById('reset-password-btn').addEventListener('click', async () => {
            alert('Password reset link sent to the tutor\'s email (Not implemented)');
        });

    } catch (error) {
        console.error("Error displaying tutor details:", error);
        container.innerHTML = `<p class="text-center text-red-500">Failed to load tutor details.</p>`;
    }
}
...
function setupTutorSearch() {
    ...
    searchInput.addEventListener('input', async (e) => {
        ...
        if (searchTerm.length > 2) {
            ...
            const snapshot = await getDocs(tutorsQuery);
            const filteredTutors = snapshot.docs.filter(doc => {
                const tutor = doc.data();
                return tutor.name.toLowerCase().includes(searchTerm) || tutor.email.toLowerCase().includes(searchTerm);
            });
...
function renderFilteredTutors(filteredDocs) {
    const tutorListContainer = document.getElementById('tutorListContainer');
    tutorListContainer.innerHTML = '';
    if (filteredDocs.length === 0) {
        tutorListContainer.innerHTML = '<p class="text-center text-gray-500">No matching tutors found.</p>';
        return;
    }
