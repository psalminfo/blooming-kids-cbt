...
async function handleStudentImport() {
    ...
    reader.onload = async (e) => {
        try {
            ...
            const expectedHeaders = ["Parent Name", "Student Name", "Grade", "Subjects", "Days", "Fee"];
            const headers = Object.keys(json[0]).filter(header => expectedHeaders.includes(header.trim()));
            ...
            for (const row of json) {
                const studentData = {
                    ...
                    days: row["Days"] || '',
                    studentFee: row["Fee"] ? parseFloat(row["Fee"]) : 0,
                    tutorEmail: tutor.email,
                    summerBreak: false,
                ...
                if (studentData.studentName && studentData.grade) {
                    batch.add(studentsCollectionRef, studentData);
                    importedCount++;
                } else {
            ...
            if (importedCount > 0) {
                await batch.commit();
                statusEl.textContent = `Successfully imported ${importedCount} student(s) for ${tutor.name}.`;
            } else {
                statusEl.textContent = "No valid students found to import.";
            }
        } catch (error) {
