...
/**
 * PHASE 1: Generates a unique referral code.
 * @returns {string} The referral code (e.g., BKH4X8T2Y).
 */
function generateReferralCode() {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    let result = 'BKH'; // Prefix for Blooming Kids House
    for (let i = 0; i < 6; i++) {
...
async function loadAllReportsForParent(parentPhone, userId) {
    ...
    try {
        ...
        // --- NEW/UPDATED: FETCH USER DATA, RETRO-GENERATE CODE, AND UPDATE PARENT NAME ---
        let userData;
        const userDoc = await db.collection('parent_users').doc(userId).get();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // NEW: Tab Listeners for Main Content
    const reportTab = document.getElementById('reportTab');
    const rewardsTab = document.getElementById('rewardsTab');
    ...
    if (rewardsTab) {
        rewardsTab.addEventListener('click', () => {
            ...
            // Re-load data if needed, though it should be loaded on sign-in
            if (auth.currentUser) {
                loadReferralRewards(auth.currentUser.uid);
            }
        });
    }
});
