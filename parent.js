    ...
    try {
        ...
        for (const key in grouped) {
            ...
            const results = session.map(testResult => {
                // This logic now correctly reads the pre-calculated score and topics.
                const topics = [...new Set(testResult.answers?.map(a => a.topic).filter(t => t))] || [];

                return {
                    subject: testResult.subject,
                    // Use the score from the database directly. Default to 0 if not found.
                    correct: testResult.score !== undefined ? testResult.score : 0,
                    // Use the total from the database directly. Default to 0 if not found.
