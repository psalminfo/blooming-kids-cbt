...
// Escape string for safe use in JavaScript inside HTML attributes
...
`).join("");
const escName = escapeJsString(fullName);
const escId = escapeJsString(sessionId);
return `
...
<button onclick="downloadSessionReport(${studentIndex}, '${escId}', '${escName}', 'assessment')"
...
<button onclick="downloadMonthlyReport(${studentIndex}, '${escId}', '${escName}')"
class="text-green-600 hover:text-green-800 font-medium flex items-center text-sm bg-green-50 px-3 py-1 rounded-lg hover:bg-green-100 transition-all duration-200">
<span class="mr-1">📥</span> Download PDF
</button>
</div>
