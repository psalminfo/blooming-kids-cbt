...
function normalizePhoneNumber(phone) {
    ...
    // Return last 10 digits for backward compatibility
...
async function handleSignUp() {
    ...
    try {
        ...
        await loadAllReportsForParent(normalizedPhone);
...
async function handleSignIn() {
    ...
    try {
        ...
        if (identifier.includes('@')) {
            ...
            // Sign in with phone - we need to find the user's email first
        ...
        // Get the normalized phone number from user profile
        const normalizedPhone = userCredential.user.displayName || 
                               (await db.collection('parent_users').doc(userCredential.user.uid).get()).data().phone;
        
        // Load all reports for the parent
        await loadAllReportsForParent(normalizedPhone);
...
async function loadAllReportsForParent(normalizedPhone) {
    ...
    try {
        ...
        const cacheKey = `reportCache_${normalizedPhone}`;
        ...
        // Query collections using the normalized parent phone number
        const assessmentQuery = db.collection("student_results").where("parentPhone", "==", normalizedPhone).get();
        const monthlyQuery = db.collection("tutor_submissions").where("parentPhone", "==", normalizedPhone).get();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            const normalizedPhone = user.displayName;
            if (normalizedPhone) {
                loadAllReportsForParent(normalizedPhone);
            }
        }
    });
