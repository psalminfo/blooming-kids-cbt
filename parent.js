...
/**
 * FINAL VERSION: Asynchronously calculates score and topics, and performs a case-insensitive comparison.
...
// Main function to load the report (Must be in the global scope)
async function loadReport() {
  ...
  try {
    ...
    for (const key in grouped) {
      ...
      const uniqueSessionTopics = [...new Set(allTopics)];
      const topicsList = uniqueSessionTopics.length > 0 
        ? `<ul>${uniqueSessionTopics.map(t => `<li class="ml-4 list-disc">${t}</li>`).join("")}</ul>`
      ...
      const fullBlock = `
        <div class="border rounded-lg shadow mb-8 p-4 bg-white" id="report-block-${blockIndex}">
          ...
          <h3 class="text-lg font-semibold mt-4 mb-2">Topics Covered</h3>
          ${topicsList}
