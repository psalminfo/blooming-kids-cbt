...
// Cache configuration - 2 weeks
const CACHE_DURATION = 14 * 24 * 60 * 60 * 1000; // 2 weeks in milliseconds

// Cache functions
function getCacheKey(parentPhone, studentName) {
    return `parent_cache_${parentPhone}_${studentName}`;
}

function saveToCache(parentPhone, studentName, data) {
    try {
        const cacheData = {
            timestamp: Date.now(),
            data: data
        };
        localStorage.setItem(getCacheKey(parentPhone, studentName), JSON.stringify(cacheData));
    } catch (error) {
        console.warn('Error saving to cache:', error);
    }
}

function loadFromCache(parentPhone, studentName) {
    try {
        const cached = localStorage.getItem(getCacheKey(parentPhone, studentName));
        ...
        if (isExpired) {
            localStorage.removeItem(getCacheKey(parentPhone, studentName));
            return null;
        }
...
async function loadReport() {
    ...
    // Check cache first
    const cachedData = loadFromCache(parentPhone, studentName);
    if (cachedData) {
        console.log('Loading from cache...');
        renderReportContent(cachedData, studentName, parentPhone);
    ...
    try {
        // OPTIMIZED SEARCH: Use normalizedPhone field for primary search
        const normalizedSearchPhone = normalizePhone(parentPhone);
        
        // Search for students by phone number first (primary search)
        ...
        studentsQuery.forEach(doc => {
            ...
            // FLEXIBLE NAME MATCHING (secondary criteria)
        ...
        if (matchingStudents.length === 0) {
            // Fallback: Check if name exists but phone doesn't match
            ...
            allStudentsSnapshot.forEach(doc => {
                ...
                if (studentData.studentName && nameMatches(studentData.studentName, studentName)) {
                    const studentPhoneDigits = studentData.parentPhone ? normalizePhone(studentData.parentPhone) : '';
                    studentsWithSameName.push({
                        name: studentData.studentName,
                        phone: studentPhoneDigits || 'No phone registered'
                    });
                }
            });
        ...
        // Get assessment reports - ONLY for the specific matched student using phone-based query
        ...
        // Get monthly reports - ONLY for the specific matched student using phone-based query
        ...
        // Save to cache
        saveToCache(parentPhone, studentName, resultData);

        renderReportContent(resultData, studentName, parentPhone);
...
function renderReportContent(data, studentName, parentPhone) {
    const { studentResults, monthlyReports, matchingStudents } = data;
    const reportContent = document.getElementById("reportContent");
    ...
    if (studentResults.length > 0) {
        ...
        for (const key in groupedAssessments) {
            ...
            if (tutorEmail && tutorEmail !== 'N/A') {
                try {
                    const tutorDoc = db.collection("tutors").doc(tutorEmail);
                    tutorDoc.get().then(tutorDoc => {
                        if (tutorDoc.exists) {
