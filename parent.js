...
window.loadReport = async function () {
  ...
  for (const key in grouped) {
    ...
    const topicRecommendations = session.map(r => {
      const topics = (r.topic || r.topics || []);
      const skills = (r.skill_detail || []).join(", ");
      const combined = topics.length ? topics.join(", ") : "General skill development recommended.";
      return `<li><strong>${r.subject.toUpperCase()}:</strong> ${combined}${skills ? ` — ${skills}` : ""}</li>`;
    }).join("");
