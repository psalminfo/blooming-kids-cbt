...
    const students = studentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), status: 'approved' }));
    
    const pendingStudentsSnapshot = await getDocs(pendingStudentQuery);
    const pendingStudents = pendingStudentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), status: 'pending' }));
...
                        <td class="px-6 py-4 whitespace-nowrap"><span class="status-indicator ${isReportSaved ? 'text-green-600 font-semibold' : 'text-gray-500'}">
                            ${student.status === 'pending' ? 'Pending Approval' : (isReportSaved ? 'Report Saved' : 'Pending Report')}
                          </span></td>
...
                    }
                    ...
                    studentsHTML += `<span class="text-sm text-gray-500">Awaiting Approval</span>`;
                }
                
                studentsHTML += `</td></tr>`;
            });
...
                const allReportsSaved = Object.keys(savedReports).length === allStudents.filter(s => !s.summerBreak).length;
                studentsHTML += `
...
                    // UPDATED: Submit to pending_students collection
                    await addDoc(collection(db, "pending_students"), {
                        ...studentData,
                        submittedByEmail: tutor.email,
...
            const student = allStudents.find(s => s.id === e.target.dataset.studentId);
