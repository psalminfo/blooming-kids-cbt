    ...
    try {
        ...
        if (studentsWithoutSchedule.length > 0) {
            ...
            showBulkSchedulePopup(studentsWithoutSchedule[0], tutor, studentsWithoutSchedule.length);
                isFirstScheduleCheck = false; // Mark that we've run a check, so subsequent manual checks will show success
            return true;
        } else {
...
function showBulkSchedulePopup(student, tutor, totalCount = 0) {
    ...
    document.getElementById('add-schedule-entry').addEventListener('click', () => {
        const scheduleEntries = document.getElementById('schedule-entries'); // Corrected variable name
    ...
    document.getElementById('save-schedule-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            await updateDoc(studentRef, { schedule: schedule }); // Ensure 'schedule' is passed as an object property
...
function showDailyTopicModal(student) {
    ...
    document.getElementById('save-topic-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            topicInput.value = '';
            await loadDailyTopicHistory(student.id); // Reload history after saving
...
function enableTopicEdit(topicId) {
    ...
    document.getElementById(`input-container-${topicId}`).classList.remove('hidden'); // Corrected ID
...
function cancelTopicEdit(topicId) {
    ...
    document.getElementById(`btn-delete-${topicId}`).classList.remove('hidden'); // Corrected ID
...
async function loadDailyTopicHistory(studentId) {
    ...
    try {
        ...
        snap.forEach(d => {
            ...
            val.parsedDate = val.createdAt?.toDate ? val.createdAt.toDate() : new Date(val.createdAt); // Ensure createdAt is a valid date
            data.push(val);
        });
...
async function uploadToCloudinary(file, studentId) {
    return new Promise((resolve, reject) => {
        ...
        formData.append('folder', 'homework_assignments'); // Ensure folder is correctly set
...
async function fetchParentEmailByPhone(phone) {
    ...
    try {
        // Query parent_users where 'phone' matches student's parentPhone (assuming phone numbers are normalized)
...
function showHomeworkModal(student) {
            ...
            <div class="modal-content max-w-2xl">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group"><label class="form-label">Files (Max 5)</label>
                        <div class="file-upload-container"><input type="file" id="hw-file" class="hidden" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.txt,.zip">
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        if (selectedFiles.length + files.length > 5) { showCustomAlert('Max 5 files allowed.'); fileInput.value=''; return; }
        ...
        try {
            ...
            if (sendEmail) {
                ...
                if (student.parentPhone) {
                    ...
                    if (!parentEmail) { // Check if parentEmail is null or empty
            ...
            for (const f of selectedFiles) {
                try {
                    ...
                    attachments.push({url:res.url, name:res.fileName, size:res.bytes, type:res.format}); // Corrected property name
                } catch(e) { 
            ...
            const hwData = {
                ...
                fileUrl: attachments[0]?.url || '', // Legacy support for single file
            ...
            if (sendEmail && parentEmail) {
                ...
                // The GAS_URL provided in the original code was incorrect. It should be updated to a valid one.
                fetch(GAS_URL, {
...
async function scheduleEmailReminder(hwData, fileUrl = '') {
    try {
        ...
        await addDoc(collection(db, "email_reminders"), { // Changed setDoc to addDoc for new documents
...
async function updateUnreadMessageCount() {
    try {
        ...
        const messagesQuery = query(
            collection(db, "tutor_messages"), // Changed to collection
                ...
                <div class="modal-body">
                    ...
                    <div class="recipient-section mb-6 hidden">
                        ...
                        <div id="group-recipient-container" class="hidden">
                            <div class="students-list-container max-h-60 overflow-y-auto border rounded-lg p-3 bg-gray-50 custom-scrollbar">
                    ...
                    <div class="message-details-section">
                        ...
                        <div class="form-group">
                            ...
                            <div class="message-editor-container">
                                ...
                                <div class="editor-tools mt-2 flex gap-2 flex-wrap">
                        ...
                        <div class="flex justify-end mb-4">
                            <button type="button" id="preview-message-btn" class="btn btn-secondary btn-sm">
                                👁️ Preview Message
                            </button>
                        </div>
                    </div>
...
function populateRecipientDropdowns(students) {
    ...
    if (parentSelect) {
        ...
        students.forEach(student => {
            ...
            option.value = student.parentPhone || ''; // Use parentPhone as value, or empty string if not available
            ...
            if (e.target.value) {
                ...
                if (studentSelect) {
                    while (studentSelect.options.length > 1) {
                        studentSelect.remove(1); // Clear previous student options
                    }
                    
                    const selectedStudentId = e.target.options[e.target.selectedIndex].getAttribute('data-student-id');
                    const student = students.find(s => s.id === selectedStudentId);
                    
                    if (student) { // Ensure student is found before adding option
...
async function loadStudentsForGroupSelection() {
    ...
    try {
        ...
        const studentsQuery = query(
            collection(db, "students"), // Corrected to collection
            where("tutorEmail", "==", tutorEmail)
        );
...
function applyTextFormatting(toolType) {
    ...
    switch(toolType) { // Corrected switch statement
        case 'bold':
        ...
        case 'underline':
            ...
            break;
        case 'list': // Added bullet list formatting
            formattedText = `• ${selectedText}`;
            break;
    }
...
function removeAttachment(index) {
    ...
    files.forEach((file, i) => { // Corrected loop variable
        if (i != index) {
            dt.items.add(file);
        }
    });
...
async function sendEnhancedMessage(modal) {
    try {
        ...
        const messageRef = doc(collection(db, "tutor_messages")); // Use addDoc for new messages
...
async function loadEnhancedConversations() {
    try {
        ...
        messagesSnapshot.docs.forEach(doc => {
            ...
            const conversationKey = message.conversationId || // Use conversationId if available
...
function getConversationTitle(message) {
    ...
    // Check for messageType first for specific titles
    if (message.messageType === 'management') {
        ...
        if (message.recipientDetails.length === 1) {
            ...
            return detail.studentName ? // Use studentName if available
...
function renderEnhancedConversationsList(conversations) {
    ...
    conversations.forEach(conv => {
        ...
        // Ensure lastMessage.createdAt is a valid Date object for sorting
            ...
            <div class="conversation-item ${conv.unread ? 'unread' : ''}" data-conversation-id="${conv.id}">
                <div class="conversation-info">
                    <div class="conversation-avatar" style="background: ${conv.lastMessage.messageType === 'management' ? '#10b981' : conv.lastMessage.messageType === 'all' ? '#f59e0b' : '#8b5cf6'};">
                        ${conv.lastMessage.messageType === 'management' ? '💼' : // Corrected icons
...
async function loadEnhancedConversationMessages(conversationId) {
    try {
        ...
        const messagesQuery = query(
            collection(db, "tutor_messages"), // Corrected to collection
            where("tutorId", "==", tutorId)
        );
        ...
        messagesSnapshot.forEach(doc => {
            ...
            const msgConversationId = message.conversationId || // Use conversationId if available
                                    (message.recipients && message.recipients[0]) || 
                                    'general';
