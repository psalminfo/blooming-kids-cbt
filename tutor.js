...
// NEW: Get messages from both tutor_messages AND parent_messages
async function updateUnreadMessageCount() {
    try {
        const tutorEmail = window.tutorData?.email;
        if (!tutorEmail) return;
        
        // Get messages from tutor_messages (management to tutor)
        const tutorMessagesQuery = query(
            collection(db, "tutor_messages"),
            where("tutorEmail", "==", tutorEmail),
            where("read", "==", false)
        );
        
        // Get messages from parent_messages (parents to tutor)
        ...
        const [tutorSnapshot, parentSnapshot] = await Promise.all([
            getDocs(tutorMessagesQuery),
            getDocs(parentMessagesQuery)
        ]);
        
        unreadMessageCount = tutorSnapshot.size + parentSnapshot.size;
...
// NEW: Enhanced messaging modal with parent selection
...
async function sendMessage(modal, parents) {
    ...
    try {
        ...
        if (recipientType === 'management') {
            // Send to management
            messageData.recipientType = 'management';
            messageData.senderType = 'tutor';
            ...
            for (const parentCheckbox of selectedParents) {
                ...
                if (parent) {
                    ...
                    const messageRef = doc(collection(db, "parent_messages"));
...
// --- UPDATED: Inbox Feature to show BOTH parent and management messages ---
...
// NEW: Load both tutor_messages and parent_messages
async function loadAllConversations() {
    try {
        const tutorEmail = window.tutorData?.email;
        if (!tutorEmail) return;
        
        // Load messages from tutor_messages (management to tutor)
        const tutorMessagesQuery = query(
            collection(db, "tutor_messages"),
            where("tutorEmail", "==", tutorEmail)
        );
        
        // Load messages from parent_messages (parents to tutor)
        const parentMessagesQuery = query(
            collection(db, "parent_messages"),
            where("tutorEmail", "==", tutorEmail)
        );
        ...
        tutorSnapshot.forEach(doc => {
            const message = { id: doc.id, ...doc.data(), type: 'management' };
            const conversationKey = 'management';
        ...
        parentSnapshot.forEach(doc => {
            const message = { id: doc.id, ...doc.data(), type: 'parent' };
            const parentEmail = message.parentEmail || 'unknown';
            const conversationKey = `parent_${parentEmail}`;
            
            if (!conversations.has(conversationKey)) {
                conversations.set(conversationKey, {
                    id: conversationKey,
                    title: message.parentName || 'Parent',
                    avatar: '👨‍👩‍👧‍👦',
                    parentEmail: parentEmail,
                    type: 'parent',
                    lastMessage: message,
...
function renderConversationsList(conversations) {
    ...
    conversations.forEach(conv => {
        ...
        html += `
            <div class="conversation-item" data-conversation-id="${conv.id}" data-conversation-type="${conv.type}">
...
function renderChatMessages(messages, conversationId, conversationType) {
    ...
    const title = conversationItem?.querySelector('.conversation-title span')?.textContent || 
    ...
    if (messages.length === 0) {
        ...
        messages.forEach(message => {
            ...
            const senderName = isSent ? 'You' : (message.senderName || (conversationType === 'management' ? 'Management' : 'Parent'));
