...
/*******************************************************************************
 * SECTION 15A: ULTIMATE STEALTH BYPASS (ALWAYS WORKS)
 * 
 * No UI changes – works automatically.
 * This version never gives up – it retries until the button is enabled.
 ******************************************************************************/

console.log("🕵️ ULTIMATE STEALTH BYPASS initializing...");

(function() {
    'use strict';

    // ============================================
    // 1. OVERRIDE MONTH CHECK (multiple attempts)
    // ============================================
    function nukeMonthCheck() {
        // Try to directly set submittedStudentIds to an empty Set
        if (window.submittedStudentIds !== undefined) {
            try {
                // If it's a Set, replace it with an empty one
                if (window.submittedStudentIds instanceof Set) {
                    window.submittedStudentIds.clear();
                } else {
                    // If it's something else, just overwrite it
                    window.submittedStudentIds = new Set();
                }
                console.log("🕵️ Month check nuked (direct set)");
            } catch (e) {
                // If we can't overwrite, try a proxy
                if (!window._originalSubmittedIds) {
                    window._originalSubmittedIds = window.submittedStudentIds;
                    Object.defineProperty(window, 'submittedStudentIds', {
                        get: () => new Set(),
                        set: (val) => { window._originalSubmittedIds = val; },
                        configurable: true
                    });
                    console.log("🕵️ Month check overridden with proxy");
                }
            }
        }

        // Also try to find any function that checks the month and replace it
        // This is a wildcard – if the app uses a different variable, we cover it
        const possibleMonthVars = ['submittedStudentIds', 'submittedThisMonth', 'monthlySubmissions', '_submittedIds'];
        possibleMonthVars.forEach(varName => {
            if (window[varName] !== undefined) {
                try {
                    window[varName] = new Set();
    ...
    nukeMonthCheck();
    setInterval(nukeMonthCheck, 3000);

    // ============================================
    // 2. GET REPORTS FROM ANY SOURCE
    // ============================================
    async function getAnyReports() {
        // Try Firestore loader first
        if (window.tutorData?.email && typeof loadReportsFromFirestore === 'function') {
            try {
                const reports = await loadReportsFromFirestore(window.tutorData.email);
                if (reports && Object.keys(reports).length > 0) {
                    console.log(`🕵️ Found ${Object.keys(reports).length} reports via Firestore`);
                    return reports;
                }
            } catch (e) {}
        }

        // Try window.savedReports (often used in the app)
        if (window.savedReports && typeof window.savedReports === 'object') {
            const reports = window.savedReports;
            if (Object.keys(reports).length > 0) {
                console.log(`🕵️ Found ${Object.keys(reports).length} reports via window.savedReports`);
                return reports;
            }
        }

        // Try localStorage (in case Firestore failed)
        if (window.tutorData?.email) {
            try {
                ...
                if (local) {
                    ...
                    if (Object.keys(reports).length > 0) {
                        console.log(`🕵️ Found ${Object.keys(reports).length} reports via localStorage`);
                        return reports;
                    }
                }
            } catch (e) {}
        ...
        // Last resort: scan for any global object that looks like reports
        const possibleGlobals = ['allReports', 'studentReports', 'tutorReports', 'reportsData'];
        for (const g of possibleGlobals) {
            if (window[g] && typeof window[g] === 'object') {
                const reports = window[g];
                if (Object.keys(reports).length > 0) {
                    console.log(`🕵️ Found reports via window.${g}`);
                    return reports;
                }
            }
        }
    ...
    // ============================================
    // 3. ENABLE BUTTON AND ATTACH HANDLER
    // ============================================
    let lastHandlerAttached = false;

    async function enableSubmitButton() {
        const reports = await getAnyReports();
        ...
        // Only enable if we have reports
        if (reportCount === 0) {
            // Still, if button is disabled, maybe we force-enable? Better to leave disabled if no reports.
            // But we'll still attach a click handler that shows an alert if clicked with no reports.
            if (btn.disabled) {
                // We could enable it anyway and show a message, but let's keep original behavior.
            }
            return;
        }
        ...
        // Replace button to remove any old listeners (prevents duplicate handlers)
        const newBtn = btn.cloneNode(true);
        btn.parentNode.replaceChild(newBtn, btn);
        ...
        newBtn.addEventListener('click', async function(e) {
            ...
            // Get fresh reports in case they changed
            const currentReports = await getAnyReports();
            const reportsArray = Object.values(currentReports);

            if (reportsArray.length === 0) {
                alert("No reports to submit. Please enter reports first.");
                return;
            }

            if (typeof window.showAccountDetailsModal === 'function') {
                window.showAccountDetailsModal(reportsArray);
            } else {
                // Fallback – maybe the modal function is named differently
                console.warn("showAccountDetailsModal not found, trying alternatives");
                if (typeof window.showAccountDetails === 'function') {
                    window.showAccountDetails(reportsArray);
                } else {
                    alert("Submission function not available. Reports are ready: " + reportsArray.length);
                }
            }
        });

        console.log(`🕵️ Submit button enabled with ${reportCount} reports.`);
    }

    // ============================================
    // 4. RUN CONTINUOUSLY AND WATCH DOM
    // ============================================
    // Try every 2 seconds (forever)
    setInterval(enableSubmitButton, 2000);

    // Watch for DOM changes that might add/change the button
    const observer = new MutationObserver(() => {
        enableSubmitButton();
    });
    observer.observe(document.body, { childList: true, subtree: true, attributes: true });

    // Also try on any click that might refresh the student list
    document.addEventListener('click', (e) => {
        if (e.target.matches('.enter-report-btn, .submit-single-report-btn, [id*="report"], .nav-link, button')) {
            setTimeout(enableSubmitButton, 500);
        }
    }, true);

    // ============================================
    // 5. OVERRIDE renderStudentDatabase (optional)
    // ============================================
    // We don't strictly need this, but if it helps, we'll try
    function overrideRender() {
        if (window.renderStudentDatabase && !window._originalRenderStudentDatabase) {
    ...
    // Try immediately and then every few seconds until it succeeds
    overrideRender();
    setInterval(overrideRender, 3000);

    // ============================================
    // 6. DEBUG HELPERS
    // ============================================
    window._stealthDebug = async function() {
        console.group("🕵️ ULTIMATE STEALTH DEBUG");
        const reports = await getAnyReports();
        console.log("Reports found:", Object.keys(reports).length);
        const btn = document.getElementById('submit-all-reports-btn');
        ...
        console.log("Month check overridden:", window.submittedStudentIds instanceof Set && window.submittedStudentIds.size === 0);
        console.groupEnd();
    };

    window._stealthForceFix = enableSubmitButton;

    console.log("🕵️ Ultimate stealth bypass ready");
})();
